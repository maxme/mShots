...
function take_snapshot( p_uri, p_filename, p_width, p_height, p_trim ) {
	let m_uri = p_uri;
	...
	(async() => {
		...
		if ( response && response.ok() || page_loaded ) {
			...
			var secondary_filename = p_filename;
			if (p_trim) {
				secondary_filename = p_filename + "_";
			}
			await page.screenshot( { path: secondary_filename, fullPage: false, type: 'jpeg', quality: 90 } );
			await page.close();
			if (p_trim) {
				await _sharp(secondary_filename).trim().toFile(p_filename);
			}
...
function check_site_queue() {
	if ( false === downloading ) {
		...
		if ( site_queue.length > 0 ) {
			...
			take_snapshot( s_details.url, s_details.file, s_details.width, s_details.height, s_details.trim );
		}
	}
}
